...
export class VotingService {
  ...
  constructor(videoRepository: VideoRepository, creatorRepository: CreatorRepository, hiveRepository: HiveRepository, configService: ConfigService) {
  ...
  async distributeVotes(hoursBetween: number) {
    ...
    let threespeak = await this.#hiveRepository.getAccount('threespeak')
  ...
  async #constructOperations(videoMap: VideoMap, totalViews: number, factor: number, hoursBetween: number): Promise<VoteOperation[1][]> {
