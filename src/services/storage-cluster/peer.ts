...
export class StorageClusterPeer extends StorageCluster {
  ...
  constructor(
    ...
    this.allocator = new StorageClusterAllocator(
      this.unionDb,
      this.secret,
      this.ipfs,
      this.getPeerId(),
  ...
  getIpfsApiUrl() {
    ...
    return (
      ipfsEndpoint.protocol +
      '//' +
      ipfsEndpoint.host +
      ':' +
      ipfsEndpoint.port +
      ipfsEndpoint['api-path']
    );
  }
  ...
  async addToCluster(cid: string | CID, metadata: PinMetadata) {
    ...
    let isPinned = false;
    for await (const pinLs of this.ipfs.pin.ls({
      type: 'recursive',
      paths: [cid],
    })) {
    ...
    if (!isPinned) {
      Logger.error(
        'Attempting to add ' + cid.toString() + " to cluster which isn't pinned",
        'storage-peer',
      );
      throw new Error('CID not pinned in node');
    }
    const isAdded = await this.pins.findOne({ _id: cid.toString() });
    if (isAdded && isAdded.status !== 'deleted') {
      Logger.error(
        cid.toString() + ' is already exists in the cluster, ignoring request',
        'storage-peer',
      );
  ...
  async unpinFromPeer(cid: string | CID) {
    if (typeof cid === 'string') cid = CID.parse(cid);
    const unpinnedTs = new Date().getTime();
  ...
  private async sendPeerInfo() {
    ...
    const totalSpaceMB = Math.floor(diskInfo.total / 1048576);
    const freeSpaceMB = Math.floor(diskInfo.available / 1048576);
    Logger.log(
      'Available disk space: ' +
        Math.floor(freeSpaceMB / 1024) +
        ' GB (' +
        Math.floor((100 * freeSpaceMB) / totalSpaceMB) +
        '%), total: ' +
        Math.floor(totalSpaceMB / 1024) +
        ' GB',
      'storage-peer',
    );
    const allocations = await this.allocator.requestAllocations({
      totalSpaceMB,
      freeSpaceMB,
    });
    if (allocations) {
  ...
  private async handlePinAlloc(allocs: SocketMsgPinAlloc, msgTs: number) {
    ...
    for (const a of allocs.allocations) {
      const exists = await this.pins.findOne({ _id: a._id, 'allocations.id': this.getPeerId() });
      if (!exists)
        await this.pins.updateOne(
          {
            _id: a._id,
    ...
    await this.executeIPFSPin(
      allocs.allocations.map((val) => val._id),
      allocs.peerIds,
      msgTs,
    );
  ...
  private async executeIPFSPin(cids: string[], peerIds: string[], allocatedTs: number) {
    ...
    for (const cid of cids) {
      ...
      try {
        const pinned = await this.ipfs.pin.add(cid);
        let size: number = 0;
        if (pinned.code === 0x70) {
          ...
          const apiRes = (
            await (
              await fetch(this.getIpfsApiUrl() + '/dag/stat?arg=' + cid, {
                method: 'POST',
              })
            ).text()
          )
        ...
        const pinnedTs = new Date().getTime();
        this.pins.updateOne(
          {
            _id: cid,
  ...
  private async pinFailed(cid: string) {
    const failedTs = new Date().getTime();
    try {
      await this.pins.updateOne(
        {
          _id: cid,
  ...
  private async responseSync(message: SocketMsgSyncResp) {
    ...
    Logger.log(
      'Synced ' + message.pins.length + ' pins and ' + message.unpins.length + ' unpins',
      'storage-cluster',
    );
    ...
    this.requestSync(
      message.pins?.length > 0
        ? message.pins[message.pins.length - 1]?.created_at ?? new Date().getTime() + 100000
        : new Date().getTime() + 100000,
      message.unpins?.length > 0
        ? message.unpins[message.unpins.length - 1]?.last_updated ?? new Date().getTime() + 100000
        : new Date().getTime() + 100000,
    );
  }
  ...
  private initWs(isDiscovery: boolean, wsUrl: string) {
    if (!wsUrl) {
      if (isDiscovery) throw new Error('no WS URL for discovered peer, this should not happen');
      this.sendPeerInfo();
      ...
      if (message.type === SocketMsgTypes.AUTH_SUCCESS) {
        ...
        if (!isDiscovery) {
          ...
          for (const p of allocPeerInfo.discoveryPeers)
            if (p !== this.wsDiscovery) this.initWs(true, p);
          setTimeout(
            async () =>
              this.requestSync(
        ...
        } else Logger.debug('Discovered peer ' + allocPeerInfo.peerId, 'storage-peer');
      }
