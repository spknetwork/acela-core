...
export class StorageClusterService {
  ...
  async start() {
    ...
    this.peer = new StorageClusterPeer(
      this.db,
      process.env.IPFS_CLUSTER_SECRET,
      this.ipfs,
      process.env.IPFS_CLUSTER_PATH,
      process.env.IPFS_CLUSTER_PEER_ID,
      process.env.IPFS_CLUSTER_WS_URL,
      process.env.IPFS_CLUSTER_WSS_PORT,
      process.env.IPFS_CLUSTER_WS_DISCOVERY,
    );
    this.peer.start();
  }
}
